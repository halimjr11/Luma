package com.halimjr11.luma.data.paging
...
class LumaRemoteMediator(
    ...
    override suspend fun load(
        ...
        try {
            val responseData = lumaService.getStories(page, state.config.pageSize)
            val endOfPaginationReached = responseData.data?.isEmpty() == true
            val mapper = responseData.data?.mapNotNull {
                it?.let { story -> localDataMapper.mapResponseToEntity(story) }
            }.orEmpty()
            database.withTransaction {
                if (loadType == LoadType.REFRESH) {
                    database.lumaDao().deleteAll()
                }
                ...
                database.lumaDao().insertQuote(mapper)
            }
